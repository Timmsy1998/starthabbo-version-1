<?php
	...
	class User {
		...
		public function login( $username, $password ) {
			...
			elseif( $num != 1 ) {
                		$db->query( "INSERT INTO logs VALUES (NULL, '{$username}', '{$_SERVER['REMOTE_ADDR']}', '{$date}', 'Attempted to login, failed due to invalid credentials.')" );
				throw new UserException( 'Invalid credentials.' );
			}
			else {
                		$db->query( "INSERT INTO logs VALUES (NULL, '{$username}', '{$_SERVER['REMOTE_ADDR']}', '{$date}', 'Successful account login.')" );
