...
<div class="wrapper">
<?php
        $query = $db->query( "SELECT username,displaygroup,habbo,djname,role FROM users WHERE displaygroup = 14 ORDER BY role DESC" );
        $num = $db->num($query);
        while( $array = $db->assoc( $query ) ) {
       
        $id = $array['id'];
        if( $array['displaygroup'] == "14" ) {
        $displaygroup = "<strong>Forum Moderator</strong>";
        }
        echo "
