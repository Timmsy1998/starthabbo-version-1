...
<div class="wrapper">
<?php
        $query = $db->query( "SELECT username,displaygroup,habbo,djname,role FROM users WHERE displaygroup = 8 ORDER BY role DESC" );
        $num = $db->num($query);
        while( $array = $db->assoc( $query ) ) {
       
        $id = $array['id'];
        if( $array['displaygroup'] == "8" ) {
        $displaygroup = "<strong>Senior Events</strong>";
        }
        echo "
