...
<div class="wrapper">
<?php
        $query = $db->query( "SELECT username,displaygroup,habbo,djname,role FROM users WHERE displaygroup = 16 ORDER BY habbo ASC" );
        $num = $db->num($query);
        while( $array = $db->assoc( $query ) ) {
       
        $id = $array['id'];
        if( $array['displaygroup'] == "16" ) {
        $displaygroup = "<strong>Ownership</strong>";
        }
        echo "
